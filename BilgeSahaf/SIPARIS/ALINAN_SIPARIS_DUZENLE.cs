...
{
    public partial class ALINAN_SIPARIS_DUZENLE : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
        public string ITEM_ID = "0";
        public ALINAN_SIPARIS_DUZENLE()
        {
            InitializeComponent();
        }
        ...
        {
            DataTable dt = helper.getDataTable("SELECT*FROM ALINAN_SIPARISLER_V2 WHERE ID=" + ITEM_ID + "");
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    DataTable dtProduct = helper.getDataTable("SELECT*FROM ALINAN_SIPARIS_URUNLER WHERE SIPARIS_ID=" + ITEM_ID + "");
                    int rowsCount = 1;
        ...
        private void sİPARİŞİSİLToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                helper.setSqlCommand("DELETE FROM ALINAN_SIPARISLER_V2 WHERE ID=" + ITEM_ID + "");
                //helper.setSqlCommand("DELETE FROM SIPARIS_TAHSILATLAR_V2 WHERE SIPARIS_ID=" + ITEM_ID + "");
                this.Close();
            }
        }
        ...
        private void kAYDETToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                helper.setSqlCommandForUpdateWithParameters("ALINAN_SIPARISLER_V2", col, val, "ID", ITEM_ID);
                string lastID = ITEM_ID;

                helper.setSqlCommand("DELETE FROM ALINAN_SIPARIS_URUNLER WHERE SIPARIS_ID=" + ITEM_ID + "");
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (row.Cells[0].Value != null && row.Cells[2].Value != null)
                    {
                        ...
                        helper.setSqlCommandForInsertWithParameters("ALINAN_SIPARIS_URUNLER", childCol, childVal);
