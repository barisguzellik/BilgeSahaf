...
{
    public partial class YENI_ALINAN_SIPARIS : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
        public YENI_ALINAN_SIPARIS()
        {
            InitializeComponent();
        }
                ...
                try
                {
                    ...
                    if (val.ToString() != string.Empty)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            dataGridView1.Rows[e.RowIndex].Cells["BEKLEME_NEDENI"].Value = "-";
                            dataGridView1.Rows[e.RowIndex].Cells["TEDARIKCI"].Value = "-";
                            dataGridView1.Rows[e.RowIndex].Cells["TEDARIK_SURESI"].Value = "-";
                        }
                    }
                }
                catch
        ...
        private void kAYDETToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                helper.setSqlCommandForInsertWithParameters("ALINAN_SIPARISLER_V2", col, val);
                string lastID = helper.getTableColumnData("SELECT TOP 1 ID FROM ALINAN_SIPARISLER_V2 ORDER BY ID DESC", "ID");
