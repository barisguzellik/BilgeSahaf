...
{
    public partial class ALIS_IADE_FATURASI_DUZENLE : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
        public ALIS_IADE_FATURASI_DUZENLE()
        {
            InitializeComponent();
        }
        ...
        private void YENI_SATIS_FATURASI_Load(object sender, EventArgs e)
        {
            ...
            DataTable dt = helper.getDataTable("SELECT*FROM ALIS_IADE_FATURALARI_V2 WHERE ID=" + ITEM_ID + "");
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dtProduct = helper.getDataTable("SELECT*FROM FUNC_GET_ALIS_IADE_FATURA_DETAY(" + ITEM_ID + ")");
            int rowsCount = 1;
        ...
        private void kAYDETToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                helper.setSqlCommandForUpdateWithParameters("ALIS_IADE_FATURALARI_V2", col, val, "ID", ITEM_ID.ToString());

                helper.setSqlCommand("DELETE FROM ALIS_IADE_FATURA_URUNLER WHERE ALIS_FATURA_ID=" + ITEM_ID + "");
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (row.Cells[0].Value != null)
                    {
                        ...
                        helper.setSqlCommandForInsertWithParameters("ALIS_IADE_FATURA_URUNLER", childCol, childVal);
        ...
        private void fATURAYISİLToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                helper.setSqlCommand("DELETE FROM ALIS_IADE_FATURALARI_V2 WHERE ID=" + ITEM_ID + "");
                helper.setSqlCommand("DELETE FROM ALIS_IADE_FATURA_URUNLER WHERE ALIS_FATURA_ID=" + ITEM_ID + "");
                this.Close();
            }
        }
