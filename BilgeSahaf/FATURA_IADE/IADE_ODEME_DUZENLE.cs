...
{
    public partial class IADE_ODEME_DUZENLE : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
        public IADE_ODEME_DUZENLE()
        {
            InitializeComponent();
        }
        ...
        private void kAYDETToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                helper.setSqlCommandForUpdateWithParameters("IADE_ODEMELER_V2", col, val, "ID", ITEM_ID.ToString());
        ...
        {
            DataTable dt = helper.getDataTable("SELECT TOP 1 *FROM IADE_ODEMELER_V2 WHERE ID=" + ITEM_ID + "");
            if (dt.Rows.Count > 0)
            {
                txtTarih.Text = dt.Rows[0]["TARIH"].ToString();
                txtTutar.Text = dt.Rows[0]["TUTAR"].ToString();
                FATURA_ID = int.Parse(dt.Rows[0]["SATIS_FATURA_ID"].ToString());
            }
        }

        private void tAHSİLATISİLToolStripMenuItem_Click(object sender, EventArgs e)
        {
            helper.setSqlCommand("DELETE FROM IADE_ODEMELER_V2 WHERE ID=" + ITEM_ID + "");
            this.Close();
        }
    }
}
