...
{
    public partial class ODEME_DUZENLE : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
        public ODEME_DUZENLE()
        {
            InitializeComponent();
        }
        ...
        private void kAYDETToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                helper.setSqlCommandForUpdateWithParameters("ODEMELER_V2", col, val, "ID", ITEM_ID.ToString());
        ...
        private void TAHSILAT_EKLE_Load(object sender, EventArgs e)
        {
            ...
            DataTable dt = helper.getDataTable("SELECT TOP 1 *FROM ODEMELER_V2 WHERE ID=" + ITEM_ID + "");
            if (dt.Rows.Count > 0)
            {
        ...
        {
            helper.setSqlCommand("DELETE FROM ODEMELER_V2 WHERE ID=" + ITEM_ID + "");
            this.Close();
        }
