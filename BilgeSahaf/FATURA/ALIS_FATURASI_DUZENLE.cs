...
{
    public partial class ALIS_FATURASI_DUZENLE : ComponentFactory.Krypton.Toolkit.KryptonForm
    {

        public ALIS_FATURASI_DUZENLE()
        {
            InitializeComponent();
        }
        ...
        private void YENI_SATIS_FATURASI_Load(object sender, EventArgs e)
        {
            ...
            DataTable dt = helper.getDataTable("SELECT*FROM ALIS_FATURALARI_V2 WHERE ID=" + ITEM_ID + "");
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dtProduct = helper.getDataTable("SELECT*FROM FUNC_GET_ALIS_FATURA_DETAY(" + ITEM_ID + ")");
            int rowsCount = 1;
        ...
        private void dataGridView1_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
            if (firstProductLoad)
            {
                ...
                try
                {
                    ...
                    if (val.ToString() != string.Empty)
                    {
                        DataTable dt = helper.getDataTable("SELECT*FROM FUNC_GET_FATURA_DETAY_WITH_ID(" + val.ToString() + ")");
                        if (dt.Rows.Count > 0)
                        {
                            var stokAdi = dt.Rows[0]["STOK_ADI"].ToString();
                            var fiyat_1 = dt.Rows[0]["FIYAT_1"].ToString();
        ...
        private void kAYDETToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                helper.setSqlCommandForUpdateWithParameters("ALIS_FATURALARI_V2", col, val, "ID", ITEM_ID.ToString());

                helper.setSqlCommand("DELETE FROM ALIS_FATURA_URUNLER WHERE ALIS_FATURA_ID=" + ITEM_ID + "");
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
        ...
        private void fATURAYISİLToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                helper.setSqlCommand("DELETE FROM ALIS_FATURALARI_V2 WHERE ID=" + ITEM_ID + "");
                helper.setSqlCommand("DELETE FROM ALIS_FATURA_URUNLER WHERE ALIS_FATURA_ID=" + ITEM_ID + "");
