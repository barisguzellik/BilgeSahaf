...
{
    public partial class STOK_GIRISI_DUZENLE : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
        public STOK_GIRISI_DUZENLE()
        {
            InitializeComponent();
        }
        ...
        private void YENI_SATIS_FATURASI_Load(object sender, EventArgs e)
        {
            ...
            DataTable dt = helper.getDataTable("SELECT*FROM STOK_GIRISLERI_V2 WHERE ID=" + ITEM_ID + "");
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    DataTable dtProduct = helper.getDataTable("SELECT*FROM STOK_GIRIS_URUNLER WHERE STOK_GIRIS_ID=" + ITEM_ID + "");
                    int rowsCount = 1;
        ...
        private void kAYDETToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                helper.setSqlCommandForUpdateWithParameters("STOK_GIRISLERI_V2", col, val, "ID", ITEM_ID.ToString());

                helper.setSqlCommand("DELETE FROM STOK_GIRIS_URUNLER WHERE STOK_GIRIS_ID=" + ITEM_ID + "");
        ...
        private void fATURAYISİLToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                helper.setSqlCommand("DELETE FROM STOK_GIRISLERI_V2 WHERE ID=" + ITEM_ID + "");
                helper.setSqlCommand("DELETE FROM STOK_GIRIS_URUNLER WHERE STOK_GIRIS_ID=" + ITEM_ID + "");
                this.Close();
            }
        }
