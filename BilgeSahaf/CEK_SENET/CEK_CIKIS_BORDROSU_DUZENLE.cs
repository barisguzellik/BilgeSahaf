...
{
    public partial class CEK_CIKIS_BORDROSU_DUZENLE : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
        public CEK_CIKIS_BORDROSU_DUZENLE()
        {
            InitializeComponent();
        }
        ...
        {
            DataTable dt = helper.getDataTable("SELECT*FROM CEK_CIKIS_BORDROSU_V2 WHERE ID=" + ITEM_ID + "");
            if (dt.Rows.Count > 0)
            {
                ...
                DataTable dt2 = helper.getDataTable("SELECT*FROM CEK_CIKIS_BORDROSU_DETAYLAR WHERE CEK_CIKIS_ID=" + ITEM_ID + "");
                int rowsCount = 1;
        ...
        private void kAYDETToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                helper.setSqlCommandForUpdateWithParameters("CEK_CIKIS_BORDROSU_V2", col, val, "ID", ITEM_ID.ToString());
                string lastID = ITEM_ID.ToString();
                helper.setSqlCommand("DELETE FROM CEK_CIKIS_BORDROSU_DETAYLAR WHERE CEK_CIKIS_ID=" + ITEM_ID + "");
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (row.Cells[1].Value != null && row.Cells[2].Value != null)
                    {
                        ...
                        helper.setSqlCommandForInsertWithParameters("CEK_CIKIS_BORDROSU_DETAYLAR", col2, val2);
        ...
        {
            helper.setSqlCommand("DELETE FROM CEK_CIKIS_BORDROSU_V2 WHERE ID=" + ITEM_ID + "");
            helper.setSqlCommand("DELETE FROM CEK_CIKIS_BORDROSU_DETAYLAR WHERE CEK_CIKIS_ID=" + ITEM_ID + "");
            this.Close();
        }
    }
